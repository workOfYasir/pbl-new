...
<head>
<title>PBL | Manage Iterations</title>
...
<li class="nav-item">
<a class="nav-link active" data-toggle="tab" href="#tab-home" role="tab">Publish New Iteration</a>
</li>
...
<h6>Add New Iteration</h6>
<form action="save-new-iteration.php" method="POST" enctype="multipart/form-data" >
Task Title:
...
<input hidden value="<?php echo $_GET['course'];?>"  autocomplete="off" type="text"  name="course"  class="form-control"  />
                         Details: <textarea   class="ckeditor" name="editor" required > </textarea>
...
<h4>Iterations | Task list &nbsp;&nbsp;&nbsp;
 <button type="button" class="btn btn-primary   float-right btn-mini btn-skew" data-toggle="modal" data-target="#Modal-tab">Add New Iteration</button></h4>
</div>
...
<li class="breadcrumb-item"><a href="#">Iteration | Task list</a>
</li>
</ul>
</div>
</div>
...
$sql = "SELECT  * FROM  iterations   where course_id='$cid' ";
...
<td>'.$row['it_title'].'</td>
<td>
<input id="date'.$row['it_id'].'" onchange="changeDate('.$row['it_id'].','.$cid.')" type="date" value="'.$row['deadline'].'" class="form-control" />
</td>
<td>
 <center>';
...
if($date > $saveDate)
{
	...
	echo '<span class="pcoded-badge label label-warning">Closed</span>';
}
else
{
	echo '<span class="pcoded-badge label label-success">Active</span>';
...
<a class="dropdown-item" href="edit-iteration.php?iteration='.$row['it_id'].'&&course='.$_GET['course'].'"><i class="icofont icofont-ui-edit"></i>Edit Task</a>
<div role="separator" class="dropdown-divider"></div>
<a class="dropdown-item" href="delete-iteration.php?iteration='.$row['it_id'].'&course='.$_GET['course'].'"><i class="icofont icofont-ui-delete"></i>Delete Task</a>
</div>
</td>
</tr>';
...
$i++;
 ...
 {
	 echo "<tr>
	 <td colspan='6'><center style='color:orange;'>No Iteration Added Yet!</center>
	 <td><tr>";
 }
?>
