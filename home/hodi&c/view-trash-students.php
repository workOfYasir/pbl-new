...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc())
     {
		...
		$sqll = "SELECT * FROM student where s_roll = '$roll' and is_dell='yes'";
 ...
 echo "<td>".$roww['s_name']."</td>";
	...
	echo "<td> 
	<a  onclick='recover_student(".'"'.$roll.'"'.")' ><button type='button' class='btn btn-success'>Recover</button> </a> 
	</td>";
	...
	<center style='color:orange'>No Record Add Yet!</center>
	</td>
	 </tr>";
}
...
function recover_student(id)
{
	
	swal({
  title: "Are you sure?",
  text: "Once Recover, Student Will be Able no Login!",
  icon: "info",
  buttons: true,
  dangerMode: false,
})
.then((willDelete) => {
  if (willDelete) {
    swal("! Your Selected Student has been Recover!", {
      icon: "success",
    });
	
	window.location="recover-single-student?student="+id;
