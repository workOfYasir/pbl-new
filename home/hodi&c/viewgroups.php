...
var ses =  document.getElementById("fall").value;
	
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.open("GET", "getCourses.php?sid=" + ses, true);
        xmlhttp.send();
	   ...
	   function show(record) {
		...
		data +='	<option value="'+record[i].c_id+'">'+record[i].c_name+'</option> ';
          
        }
	 
        document.getElementById('course').innerHTML = data;
		
		document.getElementById('course').style.background="#EBF8A4";
		
		document.getElementById('course').style.color="black";
		
		document.getElementById('fall').style.background="#EBF8A4";
		document.getElementById('section').style.background="none";
		
		document.getElementById('fall').style.color="black";
...
&nbsp&nbsp&nbspFall:&nbsp
<select style="width:110px;height:25px;"  id="fall"  onchange="getcourses()" >
...
$sql = "SELECT DISTINCT fall FROM student_course";
...
var fall =  document.getElementById("fall").value;
        ...
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.open("GET", "getSection.php?cid=" + cour +"&& fall="+ fall, true);
        xmlhttp.send();
        xmlhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
				
                sectin = JSON.parse(this.responseText);
       ...
       var data = "";
		...
		data +='	<option value="'+record[i].s_sec+'">'+record[i].s_sec+'</option> ';
          
        }
	 
        document.getElementById('section').innerHTML = data;
		
		document.getElementById('section').style.background="#EBF8A4";
...
&nbsp;&nbsp;&nbsp;Course:&nbsp;
<select style="width:120px;height:25px;"   onchange="getsection()"  id="course" >
<option value="">Select Course</option>
 
</select>

&nbsp;&nbsp;&nbsp;Section:&nbsp;
<select style="width:110px;height:25px;"   onchange="getGroups()"  id="section" >
...
var sec =  document.getElementById("section").value;
 ...
 var xmlhttp = new XMLHttpRequest();
        xmlhttp.open("GET", "getGroups.php?cid=" + cour +"&& fall="+ fall +"&& sec="+ sec, true);
        xmlhttp.send();
        xmlhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
				 
                grop = JSON.parse(this.responseText);
	...
	function showGroup(record)
	{
		 
         
        for (var i = 0; i < record.length; i++) {
           
		var mgid = record[i].groupid ;
		var mfall =  record[i].fall ;
		var mcourse =  record[i].course; 
		var msec =  record[i].section ;
		var mm = record[i].member ;
		var mproj =  record[i].proj;
	 
  data +='<div class="col-sm-4">';
 data +=' <div class="card card-border-default">';
  data +='<div class="card-header">';
  data +='<div class="row">';
 data +=' <div class="col-sm-8">';
  data +='<span style="text:60px;" class="label label-primary f-left">'+record[i].proj+'</span></div>';
  data +='<div class="col-sm-4">';
  data +='<span class="label label-default f-right">'+record[i].groupid+'</span>';
 data +='</div></div></div>';
 data +='<div class="card-block">';
 data +='<h5 >Members<br><br> </h5>';
 data +='<table>';
  data +='<tr>';
      data +=' <td scope="col"><p class="task-due"><strong style="font-size:13px" class="label label-danger">Name</strong></p></td>';
      data +=' <td> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</td> ';     
 data +='<td scope="col"><p class="task-due"><strong style="font-size:13px" class="label label-danger">Roll no</strong></p></td>';
	   data +='</tr>';
 data +='<tbody>   ';
 data +='<tr>';      
 data +='<td >Muhammad Zubair</td>';
 data +='<td></td>';
 data +='<td>'+record[i].member+'</td>';
 data +='</tr> ';
 ...
 var xmlhttp = new XMLHttpRequest();
        xmlhttp.open("GET", "getCourseName.php?cid=" + mcourse , true);
        xmlhttp.send();
        xmlhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
				 
                Mcname = JSON.parse(this.responseText);
				addshow(Mcname);
		...
		var abcc= "DataBase"
	function	addshow(record)
		{
			 
        for (var i = 0; i < record.length; i++) {
           
		   abcc = record[i].c_name;
   ...
   data +='<a >'+mfall+' | </a><a>'+msec+' | </a><a >'+abcc+'</a></div>';
 
 
 data +='<div class="col-sm-2">';
 data +='<button class="btn btn-primary btn-mini f-right" type="button" >view proposal</button>';
...
<a  name="search" class="btn-mini btn btn-primary" style="color:white" >search</a>
...
$sql = " SELECT * FROM groups group by groupid , fall , course , section  ORDER by gid ";
$result = $conn->query($sql);
...
if ($result->num_rows > 0)
{
 ...
 while($row = $result->fetch_assoc())
	 
	 {
		...
		$sqlin = "SELECT * FROM groups where  fall ='$fall' and   section ='$sec'  and course='$course'  and groupid='$grpid'";
...
<div class="row">
<div class="col-sm-8">
<span style="text:60px;" class="label label-primary f-left">'.$rowin['proj'].'</span>
</div>
<div class="col-sm-4">
<span class="label label-default f-right">'.$rowin['groupid'].'</span>
</div>
</div>
</div>
<div class="card-block">
...
<button class="btn btn-primary btn-mini f-right" type="button" >view proposal</button>
