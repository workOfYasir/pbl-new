...
include 'connection.php';
$sql = "SELECT * FROM teacher where is_dell='yes'";
...
<center> <a href="#" onclick="recover_teacher('.$rowe['t_id'].')" class="text-muted" data-toggle="tooltip" data-placement="top" title="Recover" data-original-title="Recover"><i class="icofont icofont-reply"></i>
</a></center>
</td>';
...
function recover_teacher(id)
{
	
	swal({
  title: "Are you sure?",
  text: "Once deleted, It Will Publish!",
  icon: "info",
  buttons: true,
  dangerMode: false,
})
.then((willDelete) => {
  if (willDelete) {
    swal("! Your Selected Teacher has been Recovered!", {
      icon: "success",
    });
	
	window.location="recover-single-teacher?teacher="+id;
